# NOMAGIC ATTACHMENT: CustomTWC_Cassandra4_DockerImg.zip

- attachment_id: `304001858`
- space_key: `MCS`
- parent_page_id: `304001845`
- parent_page_title: (2026x Refresh1) Installing Cassandra on Windows
- media_type: `application/zip`
- reported_bytes: 35816
- download_url: https://docs.nomagic.com/download/attachments/304001845/CustomTWC_Cassandra4_DockerImg.zip?version=1&modificationDate=1777452420320&api=v2
- payload_kind: `archive-expanded`
- downloaded_sha256: `bb460264ffdbad219c93d6ed59801131416ef8a022dd1d1f734c9d4d4948a7ed`

## ARCHIVE CONTENTS

### ENTRY: cassandra.twc.yaml
...
COPY jvm11-server.twc.options /etc/cassandra/jvm11-server.options
RUN sed -i 's/\r//g' /etc/cassandra/jvm11-server.options
COPY jvm8-server.twc.options /etc/cassandra/jvm8-server.options
RUN sed -i 's/\r//g' /etc/cassandra/jvm8-server.options
...
RUN sed -i "s/#-XX:ParallelGCThreads=16/-XX:ParallelGCThreads=$(nproc)/g" /etc/cassandra/jvm11-server.options
RUN sed -i "s/#-XX:ParallelGCThreads=16/-XX:ParallelGCThreads=$(nproc)/g" /etc/cassandra/jvm8-server.options
RUN sed -i "s/#-XX:ConcGCThreads=16/-XX:ConcGCThreads=$(nproc)/g" /etc/cassandra/jvm11-server.options
RUN sed -i "s/#-XX:ConcGCThreads=16/-XX:ConcGCThreads=$(nproc)/g" /etc/cassandra/jvm8-server.options

````

### ENTRY: jvm-server.twc.options
...
### ENTRY: jvm11-server.twc.options
...
````markdown
# Custom TWC Docker Image For Apache Cassandra

Cassandra Version: 4.1.3

The Cassandra version can be set in Dockerfile. Current configurations are not guaranteed to work with newer versions of Cassandra.

This package includes all the files and instructions for building a Cassandra container with configurations customized for TWC deployment.

The Dockerfile starts with the official DockerHub Cassandra image. The customized configuration files are then loaded into the image. Processor count for Cassandra configuration is set during image generation.

To build the image, execute `docker build` from the directory where this Dockerfile is located. The '.' indicates the Dockerfile in the current working directory. Image name and tag name are arbitrary. 

```
docker build -t <image name>:<tag name> .
```

If any of the configuration files are changed, the image will have to be rebuilt.
...
````json
{
  ...
  "type": "attachment",
  "status": "current",
  "title": "CustomTWC_Cassandra4_DockerImg.zip",
  "version": {
    "by": {
      "type": "known",
      "username": "jse21",
    ...
    "when": "2026-04-29T10:47:00.320+02:00",
    "message": "",
    "number": 1,
    "minorEdit": true,
    "hidden": false,
    "_links": {
      "self": "https://docs.nomagic.com/rest/experimental/content/304001858/version/1"
    },
    "_expandable": {
      "content": "/rest/api/content/304001858"
    }
  },
  "position": -1,
  "container": {
  ...
  "_links": {
    "download": "/download/attachments/304001845/CustomTWC_Cassandra4_DockerImg.zip?version=1&modificationDate=1777452420320&api=v2",
    "webui": "/spaces/MCS/pages/304001845/2026x+Refresh1+Installing+Cassandra+on+Windows?preview=%2F304001845%2F304001858%2FCustomTWC_Cassandra4_DockerImg.zip",
    "self": "https://docs.nomagic.com/rest/api/content/304001858"
  },
  "_expandable": {
    "operations": "",
    "children": "/rest/api/content/304001858/child",
    "restrictions": "/rest/api/content/304001858/restriction/byOperation",
    "history": "/rest/api/content/304001858/history",
    "ancestors": "",
    "body": "",
    "descendants": "/rest/api/content/304001858/descendant",
    "space": "/rest/api/space/MCS",
    "relevantViewRestrictions": "/rest/api/content/304001858/restriction/relevantViewRestrictions"
  }
}
````
