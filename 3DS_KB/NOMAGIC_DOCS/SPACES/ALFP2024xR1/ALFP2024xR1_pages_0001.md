# NOMAGIC DOCUMENTATION SPACE: Alf Plugin 2024x Refresh1

<!--NOMAGIC_SPACE key=ALFP2024xR1 chunk=1 -->

<!--NOMAGIC_PAGE id=178160548 space=ALFP2024xR1 version=1 -->
## PAGE 00001: 2024x Refresh1 Version News

- page_id: `178160548`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/178160548/2024x+Refresh1+Version+News
- version_number: 1
- version_date: `2024-07-04T13:26:24.676+02:00`
...
````html
<ac:layout><ac:layout-section ac:type="single"><ac:layout-cell><h5 style="text-align: center;">Alf Plugin</h5><p style="text-align: center;"><span style="color: rgb(128,128,128);">Released on: July 5, 2024</span></p></ac:layout-cell></ac:layout-section><ac:layout-section ac:type="single"><ac:layout-cell><p><br /></p></ac:layout-cell></ac:layout-section><ac:layout-section ac:type="single"><ac:layout-cell><p>The version was released for compatibility purposes only. It does not contain new capabilities only minor bug fixes.</p><p>Check the product compatibility <a href="https://docs.nomagic.com/display/NMDOC/Product+compatibility" style="text-decoration: none;">here</a>.</p></ac:layout-cell></ac:layout-section></ac:layout>
````

<!--NOMAGIC_PAGE id=170463369 space=ALFP2024xR1 version=1 -->
## PAGE 00002: Accessing context features without using "this"

- page_id: `170463369`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463369/Accessing+context+features+without+using+this
- version_number: 1
...
<!--NOMAGIC_PAGE id=170463364 space=ALFP2024xR1 version=1 -->
## PAGE 00004: Accessing data in Activity Edge guards

- page_id: `170463364`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463364/Accessing+data+in+Activity+Edge+guards
- version_number: 1
...
Unless the attribute *Tcontrol* is used elsewhere, its only purpose in the Activity shown in the previous example is to pass the current temperature value from the operation call to the guards. In this case, a simpler way to provide this value to the guards is to make it the decision input value for the Decision Node. In the revised Activity shown below, the result of the operation call is now provided on an Output Pin of the first Action called *Tcurrent* (via the shortcut for [CONFLUENCE_PAGE title='Using Alf expressions in Opaque Actions' space='ALFP2024xR1']). This Output Pin is then connected to the Decision Node by an Object Flow, so the value on the Output Pin becomes the decision input value for the Decision Node. This value can then be accessed in the guard expressions using the name of the Output Pin.
...
<!--NOMAGIC_PAGE id=170463292 space=ALFP2024xR1 version=1 -->
## PAGE 00007: Address Book

- page_id: `170463292`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463292/Address+Book
- version_number: 1
...
In the script editor window, the language must be set to "Alf Script". The scripting window has an automatic syntax and constraint check, the same as [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'][CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'].
...
<!--NOMAGIC_PAGE id=170463294 space=ALFP2024xR1 version=1 -->
## PAGE 00010: Alf project template

- page_id: `170463294`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463294/Alf+project+template
- version_number: 1
...
This tutorial describes how to create an Alf project containing a very simple "Hello World" activity that can be executed using Cameo Simulation Toolkit. (This Activity is similar to, but even simpler than, the one in the [CONFLUENCE_PAGE title='Hello' space='ALFP2024xR1'] sample model.)
...
This tutorial describes how to create an Alf project containing a simple *Address Book* model, along with a test**Activity that can be executed using Cameo Simulation Toolkit. (This model is similar to, but even simpler than, the one in the [CONFLUENCE_PAGE title='Address Book' space='ALFP2024xR1'] sample model.)
...
<!--NOMAGIC_PAGE id=170463422 space=ALFP2024xR1 version=2 -->
## PAGE 00013: Alf Tutorial 3: Stopwatch

- page_id: `170463422`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463422/Alf+Tutorial+3+Stopwatch
- version_number: 2
- version_date: `2024-05-08T13:24:57.890+02:00`
- ancestors: Alf Plugin Documentation > Tutorials
- labels: []
...
<!--NOMAGIC_PAGE id=170463388 space=ALFP2024xR1 version=1 -->
## PAGE 00014: Building and cleaning

- page_id: `170463388`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463388/Building+and+cleaning
- version_number: 1
...
As discussed in [CONFLUENCE_PAGE title='Dependency Management' space='ALFP2024xR1'], when you make changes within a model, the Alf compiler normally automatically processes any Alf code that might be affected by these changes. However, in some cases the Alf compiler may not have enough information to realize that a change in the model will correct an error in previously erroneous Alf code. Or, you may have turned off the [CONFLUENCE_PAGE title='Environment options' space='ALFP2024xR1']**Build Automatically** environment option, in which case the Alf compiler will mark elements needing recompilation without automatically recompiling them.
...
- **Build Project**[CONFLUENCE_PAGE title='Building and cleaning' space='ALFP2024xR1'] – A normal build triggers the recompilation of the Alf bodies of all model elements marked as having Alf compilation errors or otherwise needing to be recompiled.
- [CONFLUENCE_PAGE title='Building and cleaning' space='ALFP2024xR1'] – A clean build triggers the recompilation of all Alf code in your project, whether it is marked as needing recompilation or not.

##### build-projectBuild Project

To initiate a normal build of your project, select **Tools > Alf > Build Project**. If there are any elements in your project that are marked with either an Alf compilation error annotation or an Alf recompilation-needed warning annotation (see [CONFLUENCE_PAGE title='Dependency Management' space='ALFP2024xR1']), then the Alf bodies for these elements will be recompiled. Any previously erroneous Alf code that compiles successfully has its error annotation removed, while any code with new compilation errors has a new error annotation added. All recompilation-needed warning annotations are removed.
...
The figure below shows the error annotations caused by deleting the Association in the [CONFLUENCE_PAGE title='Address Book' space='ALFP2024xR1'] example and then recreating it as a new Association, so the error annotations remain (see also the discussion of this example in [CONFLUENCE_PAGE title='Dependency Management' space='ALFP2024xR1']). Selecting **Tools > Alf > Build Project** initiates an Alf build of the project, resulting in the successful recompilation of the indicated operations and the removal of the error annotations.
...
Compilation of template bindings in your Alf code result in the generation of instantiated Classes in the [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1']*$$Template Bindings* Package in your project. Over time, Classes in this Package may accumulate that are no longer necessary for your project. A clean rebuild first cleans out this Package, so, at the end of the build, it contains only the instantiations truly needed for the Alf code actually in your project. If your Alf code no longer has any template bindings, then the Package is removed entirely from your model.
...
<!--NOMAGIC_PAGE id=170463373 space=ALFP2024xR1 version=1 -->
## PAGE 00016: Checking the State of an object

- page_id: `170463373`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463373/Checking+the+State+of+an+object
- version_number: 1
...
<!--NOMAGIC_PAGE id=170463377 space=ALFP2024xR1 version=1 -->
## PAGE 00017: Compiling Alf

- page_id: `170463377`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463377/Compiling+Alf
- version_number: 1
...
<!--NOMAGIC_PAGE id=170463415 space=ALFP2024xR1 version=1 -->
## PAGE 00018: Creating AddressBook operations

- page_id: `170463415`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463415/Creating+AddressBook+operations
- version_number: 1
...
3. Click on the new put Operation, and, in the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window, press Create .
...
A project in which Alf is to be used should be created from the [CONFLUENCE_PAGE title='Alf project template' space='ALFP2024xR1'], to ensure that all necessary library models are included.
...
6. In the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'], type the statements shown in the following figure. [ATTACHMENT filename='Address_Book-Creating_Entry_Constructor.png']
...
Next: [CONFLUENCE_PAGE title='Creating AddressBook operations' space='ALFP2024xR1']
...
1. Create a new Alf project as described in [CONFLUENCE_PAGE title='Alf project template' space='ALFP2024xR1'] , using the name Address Book for the project.
...
Next: [CONFLUENCE_PAGE title='Creating Entry attributes' space='ALFP2024xR1']
...
2. In the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window, type *WriteLine("Hello World!");*``
...
Next: [CONFLUENCE_PAGE title='Running the Hello World activity' space='ALFP2024xR1']

#### PANEL: Related pages

Related pages

- [CONFLUENCE_PAGE title='Working with Alf' space='ALFP2024xR1']
  - [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1']
  - [CONFLUENCE_PAGE title='Using Alf to define Activities' space='ALFP2024xR1']
- [CONFLUENCE_PAGE title='Compiling Alf' space='ALFP2024xR1']
  - [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1']

### EXACT CONFLUENCE STORAGE SOURCE

````html
<ac:layout><ac:layout-section ac:type="two_right_sidebar"><ac:layout-cell><p>To define a new activity using Alf, first the Activity needs to be created in the model, and then Alf code needs to be written for it and compiled.</p><p><br /></p><p>To create the <em>Hello World</em> Activity</p><hr /><ol><li>Right click on the root <em>Model</em> and select <strong>Create Element</strong> &gt; <strong>Activity</strong>.</li><li><p>Enter the name <em>Hello World</em>.</p><p class="auto-cursor-target"><ac:image><ri:attachment ri:filename="Hello_World-Creating_Activity.png"><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Creating the Hello World activity" /></ri:attachment></ac:image></p></li></ol><p><br /></p><p>To create an Alf body for <em>Hello World</em></p><hr /><ol><li><p class="auto-cursor-target">Select the new Activity in the Model Browser and open the Alf editor window (select <strong>Windows &gt; Alf</strong>), if it isn't already open.</p></li><li><p class="auto-cursor-target">In the <ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="The Alf editor" /><ac:plain-text-link-body><![CDATA[Alf editor]]></ac:plain-text-link-body></ac:link> window, type <em>WriteLine(&quot;Hello World!&quot;);</em><code> <br /></code></p></li><li>If there are any errors in your code, fix them.</li><li><p class="auto-cursor-target">When the code is entered correctly, click the <strong>Save</strong> button. The Alf code will be compiled into Activity Nodes and Edges within the selected Activity.<br /><br /><ac:image><ri:attachment ri:filename="Hello_World-Creating_Alf_Body.png"><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Creating the Hello World activity" /></ri:attachment></ac:image></p></li></ol><p><br /></p><p>Next: <ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Running the Hello World activity" /></ac:link></p></ac:layout-cell><ac:layout-cell><p class="auto-cursor-target"><br /></p><ac:structured-macro ac:name="panel" ac:schema-version="1" ac:macro-id="ba3cafe6-d881-42db-b28d-c9cc4b02bda0"><ac:parameter ac:name="title">Related pages</ac:parameter><ac:rich-text-body><ul><li><ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Working with Alf" /></ac:link><ul><li><ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="The Alf editor" /></ac:link></li><li class="confluence-link"><ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Using Alf to define Activities" /></ac:link></li></ul></li><li><ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Compiling Alf" /></ac:link><ul><li><ac:link><ri:page ri:space-key="ALFP2024xR1" ri:content-title="The Alf compiler" /></ac:link></li></ul></li></ul></ac:rich-text-body></ac:structured-macro><br /><p> </p></ac:layout-cell></ac:layout-section></ac:layout>
````

<!--NOMAGIC_PAGE id=170463428 space=ALFP2024xR1 version=1 -->
## PAGE 00024: Creating the StopWatch state machine

- page_id: `170463428`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463428/Creating+the+StopWatch+state+machine
- version_number: 1
...
First, use the [CONFLUENCE_PAGE title='Alf project template' space='ALFP2024xR1'] to create a new project called *StopWatch.*Next, define a stopwatch Class and its attributes using a Class diagram.
...
Next: [CONFLUENCE_PAGE title='Creating the StopWatch state machine' space='ALFP2024xR1']
...
Next: [CONFLUENCE_PAGE title='Running the Stopwatch model' space='ALFP2024xR1']
...
3. While resetTime operation is selected, open the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window (select Windows > Alf ), if it isn't already open, and press Create .
...
3. Click on the *increaseTime* operation and, in the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window, press **Create**.
...
Next: [CONFLUENCE_PAGE title='Defining the Ready and Running behaviors using Alf' space='ALFP2024xR1']
...
- Build On Project Open – The default for this option is false, in which case, when a project with Alf code is opened, the Alf compiler parses all the code in the project in order to [CONFLUENCE_PAGE title='Dependency Management' space='ALFP2024xR1'] of the code on other model elements. Set this option to true to automatically perform a [CONFLUENCE_PAGE title='Building and cleaning' space='ALFP2024xR1'] on a project when it is open, which not only parses all the Alf code, but also recompiles it.
- Build Automatically – The default for this option is true, in which case the Alf compiler will automatically recompile Alf code whenever it may be necessary due to changes in model elements on which the code depends. Set this option to false to prevent automatic recompilation, in which case the Alf compiler will instead just mark elements having Alf code needing recompilation with [CONFLUENCE_PAGE title='Dependency Management' space='ALFP2024xR1'] .
...
<!--NOMAGIC_PAGE id=170463291 space=ALFP2024xR1 version=1 -->
## PAGE 00031: Hello

- page_id: `170463291`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463291/Hello
- version_number: 1
...
Both these activities were compiled from Alf. To see the original Alf code, open the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window (select **Window > Alf**) and select either activity. The Alf code will appear in the window.

When [CONFLUENCE_PAGE title='Running a model with Alf' space='ALFP2024xR1'], the *Hello World* should write "Hello World!" in the console pane of the simulation window.
...
If you have projects open, and you want to import Alf into a new project, but don't want to close the currently open projects, then first use **File > New Project** to create a [CONFLUENCE_PAGE title='Alf project template' space='ALFP2024xR1'], and import the Alf file into that, as above.
...
}]]></ac:plain-text-body></ac:structured-macro><p>Importing the example above into MagicDraw results in the model structure shown below. Note that not diagrams are created by the import process. You will need to create any diagrams by hand once the import is completed.</p><p><ac:image ac:align="center" ac:width="400"><ri:attachment ri:filename="Importing_Alf-Address_Book_Model.png"><ri:page ri:space-key="ALFP2024xR1" ri:content-title="Importing Alf" /></ri:attachment></ac:image></p><h6 style="text-align: center;">Imported Address Book Model</h6><p><br /></p></ac:layout-cell><ac:layout-cell><p> </p></ac:layout-cell></ac:layout-section><ac:layout-section ac:type="two_equal"><ac:layout-cell><p><br /></p></ac:layout-cell><ac:layout-cell><p> </p></ac:layout-cell></ac:layout-section></ac:layout>
````

<!--NOMAGIC_PAGE id=170463285 space=ALFP2024xR1 version=2 -->
## PAGE 00034: Installation

- page_id: `170463285`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463285/Installation
- version_number: 2
- version_date: `2024-05-08T13:24:57.503+02:00`
- ancestors: Alf Plugin Documentation > User Guide > Getting Started
- labels: []
...
The Alf Plugin of the current version requires that [Cameo Simulation Toolkit](https://docs.nomagic.com/display/CST2024xR1/Installation+and+licensing) of the current version is already installed.
...
[CONFLUENCE_PAGE title='Using Alf for classifier behaviors' space='ALFP2024xR1']
...
3. Click on Test , and open the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window (select Windows > Alf ), if it isn't already open [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1']
...
When you delete the code for a template binding with an Alf body, the instantiated class generated from that Alf code is *not* automatically deleted from the *$$Template Bindings* package, since that class might also be referenced elsewhere in your model. Thus, if you use templates in your Alf code, it is possible, over time, for the *$$Template Bindings* package to accumulate contents that are no longer useful. If necessary, this can be cleaned up by doing a *[CONFLUENCE_PAGE title='Building and cleaning' space='ALFP2024xR1']* of your project.
...
<!--NOMAGIC_PAGE id=170463282 space=ALFP2024xR1 version=1 -->
## PAGE 00046: User Guide

- page_id: `170463282`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463282/User+Guide
- version_number: 1
...
<!--NOMAGIC_PAGE id=170463356 space=ALFP2024xR1 version=1 -->
## PAGE 00047: Using Alf expressions in Opaque Actions

- page_id: `170463356`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463356/Using+Alf+expressions+in+Opaque+Actions
- version_number: 1
...
You can use the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf Expression for the guard for an Activity Edge (Object Flow or Control Flow) in an Activity or to edit an existing guard Expression.
...
- [CONFLUENCE_PAGE title='Accessing data in Activity Edge guards' space='ALFP2024xR1']
- [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1']
- [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1']
...
In UML, the behavior of an Operation of a Class can be defined using a Behavior element called the *method* of the Operation. You can use the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf body for a new Operation method or to edit the existing Alf body of a method.
...
- Select the Operation in the Model Browser or on a Class diagram and open the Alf editor window (select **Windows > Alf**), if it isn't already open. The existing Alf code will appear in the window. The Alf code for an Operation is actually stored as the Alf body of the method Behavior of the Operation (which is an Activity or Opaque Behavior, as created above). It is therefore possible to directly the edit the code on the method Behavior (as described in [CONFLUENCE_PAGE title='Using Alf to define Behaviors' space='ALFP2024xR1']), but it is usually easier to act directly on the Operation. [IMAGE alt='' src='']
...
You can use the[CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf body for a new default value for a Property (e.g., the attribute of a Class) or to edit the existing Alf body of a Property. The Alf expression for a default value is stored as the the body of an Opaque Expression with the language *Alf.* Compiling this Alf code results in the creation of an executable Activity that returns the value of the Alf expression. This Activity is linked to the Opaque Expression as its the **Behavior** property. (See also the discussion on [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1'])
...
4. Enter the Alf code for the Behavior and press **Save** to compile and save it. The Alf code for a State Behavior can access attribute values of the context Classifier of the State Machine containing its State (using Alf *this* expressions). It can also [CONFLUENCE_PAGE title='Accessing Event data' space='ALFP2024xR1'] in the Event occurrence that triggered its execution (e.g., the attribute data of a received Signal).
...
You can use the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf body for the effect Behavior of a Transition in a State Machine or to edit the existing Alf body of an effect Behavior.
...
4. Enter the Alf code for the Behavior and press **Save** to compile and save it. The Alf code for an effect Behavior can access attribute values of the context Classifier of the State Machine containing its Transition (using Alf *this* expressions). It can also [CONFLUENCE_PAGE title='Accessing Event data' space='ALFP2024xR1'] in the Event occurrence that triggered its execution (e.g., the attribute data of a received Signal).
...
[IMAGE alt='' src=''] Even while you are editing an existing effect Behavior, the **Create** button stays active if the Transition does not have a guard. Press **Create** in order to [CONFLUENCE_PAGE title='Using Alf for Transition guard Expressions' space='ALFP2024xR1'] on the Transaction.
...
You can use the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf Expression for the guard of a Transition in a State Machine or to edit an existing guard Expression.
...
<!--NOMAGIC_PAGE id=170463313 space=ALFP2024xR1 version=1 -->
## PAGE 00056: Using Alf in Class models

- page_id: `170463313`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463313/Using+Alf+in+Class+models
- version_number: 1
...
<!--NOMAGIC_PAGE id=170463349 space=ALFP2024xR1 version=1 -->
## PAGE 00057: Using Alf in Opaque Action bodies

- page_id: `170463349`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463349/Using+Alf+in+Opaque+Action+bodies
- version_number: 1
...
[CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] may be used to create an Alf body for a new Opaque Behavior or to edit the existing Alf body of an Opaque Behavior. The Alf code for an Opaque Behavior is stored as the body of the Opaque Behavior with the language *Alf.* Compiling this Alf code results in the creation of an executable Activity reflecting the behavior defined by the Alf code. This Activity is attached to the Opaque Behavior as a tagged value of the special *CompiledRepresentation* stereotype. (See also the discussion on [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1'].)
...
<!--NOMAGIC_PAGE id=170463306 space=ALFP2024xR1 version=1 -->
## PAGE 00060: Using Alf to define Activities

- page_id: `170463306`
- space_key: `ALFP2024xR1`
- source_url: https://docs.nomagic.com/spaces/ALFP2024xR1/pages/170463306/Using+Alf+to+define+Activities
- version_number: 1
...
You can use the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] to create an Alf body for a new Activity or to edit the existing Alf body of an Activity. Compiling the Alf body of an Activity results in the generation of Activity Nodes and Activity Edges within the Activity. Executing the Activity then has the behavior specified by the original Alf code. The Alf code itself is stored in a Comment owned by the Activity, which has the *TextualRepresentation* stereotype applied. Normally, you should not modify any of the contents of an Activity with an Alf body, other than by editing its Alf code using the Alf editor. (See also the discussion on [CONFLUENCE_PAGE title='The Alf compiler' space='ALFP2024xR1'].)
...
Two kinds of UML Behaviors may directly have Alf bodies: Activities and Opaque Behaviors. You can edit the Alf body of either kind of Behavior using the [CONFLUENCE_PAGE title='The Alf editor' space='ALFP2024xR1'] window. You can also edit the body of an Opaque Behavior directly through its Specification.
