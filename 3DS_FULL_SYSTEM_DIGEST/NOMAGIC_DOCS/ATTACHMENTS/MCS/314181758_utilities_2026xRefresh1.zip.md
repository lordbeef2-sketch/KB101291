# NOMAGIC ATTACHMENT: utilities_2026xRefresh1.zip

- attachment_id: `314181758`
- space_key: `MCS`
- parent_page_id: `304002672`
- parent_page_title: (2026x Refresh1) Downloadable scripts
- media_type: `application/zip`
- reported_bytes: 14601
- download_url: https://docs.nomagic.com/download/attachments/304002672/utilities_2026xRefresh1.zip?version=1&modificationDate=1782458055005&api=v2
- payload_kind: `archive-expanded`
- downloaded_sha256: `2aaf6cab6cd2168a3519fc6227552b562e1aa490b340e0500cc109ec0a789542`
...
### ENTRY: cassandra-applied-settings.sh
...
````text
#!/bin/bash
# Cassandra Installer Settings Extractor (v4 & v5 compatible)
# Uses version detection to select conf directory and JVM file

OUTPUT_FILE="/tmp/cassandra_settings_report.txt"
TUNE_MARKER="#  Preliminary tunings for Teamwork Cloud"
SYSCTL_FILE="/etc/sysctl.conf"
FIREWALL_SERVICES_FILE="/etc/firewalld/services/cassandra.xml"

echo "Extraction started..."
echo "Cassandra Settings Extraction Report" > "$OUTPUT_FILE"
echo "Generated: $(date)" >> "$OUTPUT_FILE"
echo "=========================================================" >> "$OUTPUT_FILE"

# ----------- Determine Cassandra version ------------
CASS_VERSION=$(cassandra -v 2>/dev/null || echo "")

if [[ $CASS_VERSION == 5* ]]; then
    CASS_CONF="/etc/cassandra/default.conf"
    JVM_FILE="$CASS_CONF/jvm17-server.options"
elif [[ $CASS_VERSION == 4* ]]; then
    CASS_CONF="/etc/cassandra/conf"
    JVM_FILE="$CASS_CONF/jvm11-server.options"
else
    echo "Unsupported Cassandra version or not installed. Found version: '$CASS_VERSION'" | tee -a "$OUTPUT_FILE"
    exit 1
fi

# ----------- Cassandra YAML ------------
if [ -f "$CASS_CONF/cassandra.yaml" ]; then
    echo "From $CASS_CONF/cassandra.yaml:" >> "$OUTPUT_FILE"
    echo "=========================================================" >> "$OUTPUT_FILE"

    # Extract installer-modified keys
    grep -E "^(cluster_name:|num_tokens:|\\s*- seeds:|listen_address:|rpc_address:|broadcast_rpc_address:|broadcast_address:|endpoint_snitch:|commitlog_directory:|saved_caches_directory:|commitlog_segment_size:|commitlog_total_space:|read_request_timeout:|write_request_timeout:|truncate_request_timeout:|request_timeout:|batch_size_warn_threshold:|batch_size_fail_threshold:|hints_directory:)" "$CASS_CONF/cassandra.yaml" >> "$OUTPUT_FILE"

    # Capture YAML arrays like data_file_directories
    awk '
    /^data_file_directories:/ {flag=1; print; next}
    flag && /^[[:space:]]+-/ {print; next}
    flag && !/^[[:space:]]+-/ {flag=0}
    ' "$CASS_CONF/cassandra.yaml" >> "$OUTPUT_FILE"
...
if [ -f "$JVM_FILE" ]; then
    echo "=========================================================" >> "$OUTPUT_FILE"
    echo "From $JVM_FILE:" >> "$OUTPUT_FILE"
    echo "=========================================================" >> "$OUTPUT_FILE"
    grep -E "UseG1GC|ParallelGCThreads|ConcGCThreads" "$JVM_FILE" | grep -v "#" >> "$OUTPUT_FILE"
    echo >> "$OUTPUT_FILE"
else
    echo "JVM options file not found: $JVM_FILE" >> "$OUTPUT_FILE"
    echo >> "$OUTPUT_FILE"
fi

# ----------- Logback ------------
if [ -f "$CASS_CONF/logback.xml" ]; then
    echo "============================================" >> "$OUTPUT_FILE"
    echo "From $CASS_CONF/logback.xml:" >> "$OUTPUT_FILE"
    echo "============================================" >> "$OUTPUT_FILE"
    echo "In order to disable debug.log, comment-out the ASYNCDEBUGLOG" >> "$OUTPUT_FILE"
    grep -E "ASYNCDEBUGLOG" "$CASS_CONF/logback.xml" >> "$OUTPUT_FILE"
    echo >> "$OUTPUT_FILE"
fi

# ----------- Sysctl Tunings ------------
if grep -q "$TUNE_MARKER" "$SYSCTL_FILE"; then
    echo "============================================" >> "$OUTPUT_FILE"
    echo "Sysctl tunings applied by installer:" >> "$OUTPUT_FILE"
    echo "============================================" >> "$OUTPUT_FILE"
    grep -A50 "$TUNE_MARKER" "$SYSCTL_FILE" | grep -E "net\.|vm\." >> "$OUTPUT_FILE"
    echo >> "$OUTPUT_FILE"
fi

# ----------- Firewall ------------
if [ -f "$FIREWALL_SERVICES_FILE" ]; then
    echo "============================================" >> "$OUTPUT_FILE"
    echo "Firewall service ports configured by installer:" >> "$OUTPUT_FILE"
    echo "============================================" >> "$OUTPUT_FILE"
    grep -E "port=" "$FIREWALL_SERVICES_FILE" >> "$OUTPUT_FILE"
    echo >> "$OUTPUT_FILE"
fi
...
````json
{
  ...
  "type": "attachment",
  "status": "current",
  "title": "utilities_2026xRefresh1.zip",
  "version": {
    "by": {
      "type": "known",
      "username": "jse21",
    ...
    "when": "2026-06-26T09:14:15.005+02:00",
    "message": "",
    "number": 1,
    "minorEdit": true,
    "hidden": false,
    "_links": {
      "self": "https://docs.nomagic.com/rest/experimental/content/314181758/version/1"
    },
    "_expandable": {
      "content": "/rest/api/content/314181758"
    }
  },
  "position": -1,
  "container": {
  ...
  "_links": {
    "download": "/download/attachments/304002672/utilities_2026xRefresh1.zip?version=1&modificationDate=1782458055005&api=v2",
    "webui": "/spaces/MCS/pages/304002672/2026x+Refresh1+Downloadable+scripts?preview=%2F304002672%2F314181758%2Futilities_2026xRefresh1.zip",
    "self": "https://docs.nomagic.com/rest/api/content/314181758"
  },
  "_expandable": {
    "operations": "",
    "children": "/rest/api/content/314181758/child",
    "restrictions": "/rest/api/content/314181758/restriction/byOperation",
    "history": "/rest/api/content/314181758/history",
    "ancestors": "",
    "body": "",
    "descendants": "/rest/api/content/314181758/descendant",
    "space": "/rest/api/space/MCS",
    "relevantViewRestrictions": "/rest/api/content/314181758/restriction/relevantViewRestrictions"
  }
}
````
