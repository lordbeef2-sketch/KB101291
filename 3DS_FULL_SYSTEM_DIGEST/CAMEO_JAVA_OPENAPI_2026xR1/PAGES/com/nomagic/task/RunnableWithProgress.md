# JAVA OPENAPI: RunnableWithProgress (2026x Refresh1 API)

- source_url: https://jdocs.nomagic.com/2026xRefresh1/com/nomagic/task/RunnableWithProgress.html
- source_path: `com/nomagic/task/RunnableWithProgress.html`
- source_sha256: `d3481c74fd44875f191d0e8a1393aeba98531c5137f7b99cf69b2b05a8a4b871`
- captured_utc: `2026-07-14T16:46:16.491020+00:00`
...
// create runnable
 ...
 for (int i = 0; i invalid input: '<' max; ++i)
 {
...
<pre>
        ...
        RunnableWithProgress runnable = new RunnableWithProgress()
    {
                public void run(ProgressStatus progressStatus)
        {
                        int max = Integer.MAX_VALUE;
                        progressStatus.init("Counting...", 0, max);
                        for (int i = 0; i <span class="invalid-tag">invalid input: '&lt;'</span> max; ++i)
            {
                                if (progressStatus.isCancel())
                {
                                        // cancel clicked
